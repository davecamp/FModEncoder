...
void CFModEncoderCodecMp3::Close(){
	FreeLibrary(m_hLameDll);
	m_hLameDll = 0;
}

int CFModEncoderCodecMp3::InitCodec(int iNoOfChannels,float dwInSamplerate, float dwOutBitrate, void* pInitData, unsigned long* puiInitDataSize){
	m_fnLameSetNumberChannels(m_pLame,iNoOfChannels);
	m_fnLameSetInSamplerate(m_pLame,static_cast<int>(dwInSamplerate));
	m_fnLameSetBitrate(m_pLame,dwOutBitrate);
	m_fnLameSetMode(m_pLame,STEREO);
...
int CFModEncoderCodecMp3::Encode(void* pInSamples,unsigned long uiInSamplesLength, void* pEncodedSamples, unsigned long* uiEncodedSamplesLength){
...
void CFModEncoderCodecMp3::EmptyCodec(void* pEncodedSamples, unsigned long* uiEncodedSamplesLength){
	BYTE* pbEncodedSamples = static_cast<BYTE*>(pEncodedSamples);

	*uiEncodedSamplesLength = m_fnLameFlushEncoder(m_pLame,pbEncodedSamples,0);
}

int CFModEncoderCodecMp3::InitLameDll(){
